...
keys.addEventListener('click', e => {
    if (e.target.matches('button')){
        ...
        if(!action) { // if not an operator key
            if (screenDisplay === '0' || lastKey === 'operator') { // if 0 or an operator
            ...
            calculator.dataset.firstNum = screenDisplay; // grab first input number
            calculator.dataset.operator = action; // grab operator value
